...
export default async function Today() {
  ...
  // Read-only: the Today screen shows the world but never grants anything. Awards are written on
  // /quest, so opening the home page cannot quietly change somebody's level while they read it.
  const game = await journeyState();
  ...
  return (
    <div className="page">
      ...
      {/* ---------------- life quest ---------------- */}
      <Link href="/" className="block mt-6 card overflow-hidden hover:shadow-[var(--shadow-lift)] transition-shadow">
        <div className="relative" data-world={game.theme.key}>
          <World w={game.world} theme={game.theme.key} className="!rounded-none" />
          <div className="absolute inset-0 p-4 md:p-6 flex flex-col justify-between">
            <div>
              <div className="eyebrow on-film">Life Quest</div>
              <div className="on-film font-display text-2xl md:text-3xl mt-0.5">{game.level.region.name}</div>
            </div>
            <div className="on-film text-sm">
              Level {game.level.level} · {game.xp.toLocaleString()} XP
              {game.gems > 0 ? ` · ${game.gems} progress gem${game.gems === 1 ? "" : "s"}` : ""}
            </div>
          </div>
        </div>
      </Link>
